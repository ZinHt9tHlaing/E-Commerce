...
interface ImageUploadProps {
  photos: Array<{ url: string; file?: File; public_alt?: string }>;
  onChange: (
    images: Array<{ url: string; file?: File; public_alt?: string }>
  ) => void;
}

const ImageUpload = ({ photos, onChange }: ImageUploadProps) => {
  ...
  const removeImage = (index: number) => {
    ...
    if (photos[index].url.startsWith("blob")) {
      const objectUrl = photos[index].url;
